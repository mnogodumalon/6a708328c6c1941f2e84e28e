 ...
 * schichtvorlagenList (full hook array — resolves the Schichtvorlagen applookup),
...
import type { Schichtplan, Mitarbeiter, Schichtvorlagen, LookupValue } from '@/types/app';
...
export type SchichtplanDialogDefaults = Omit<Schichtplan['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['schichtplan']?.['status'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  mitarbeiter: APP_IDS.MITARBEITER,
  schichtvorlage: APP_IDS.SCHICHTVORLAGEN,
};
...
export function SchichtplanDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeiterList, schichtvorlagenList, enablePhotoScan = true, enablePhotoLocation = true }: SchichtplanDialogProps) {
  ...
  // Inline-Create state for "Schichtvorlagen" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraSchichtvorlagen` list, and select it in
  // the originating Combobox via the captured `createSchichtvorlagenField`.
  const [createSchichtvorlagenOpen, setCreateSchichtvorlagenOpen] = useState(false);
  const [createSchichtvorlagenInitial, setCreateSchichtvorlagenInitial] = useState('');
  const [createSchichtvorlagenField, setCreateSchichtvorlagenField] = useState<string>('');
  const [extraSchichtvorlagen, setExtraSchichtvorlagen] = useState< Schichtvorlagen[]>([]);
  const schichtvorlagenListAll = useMemo(
    () => [...schichtvorlagenList, ...extraSchichtvorlagen],
    [schichtvorlagenList, extraSchichtvorlagen],
  );
  function openCreateSchichtvorlagen(fieldKey: string, q: string) {
    setCreateSchichtvorlagenField(fieldKey);
    setCreateSchichtvorlagenInitial(q);
    setCreateSchichtvorlagenOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['datum', 'mitarbeiter', 'schichtvorlage'] as const;
  ...
  }), [mitarbeiterList, schichtvorlagenList, ]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">Einsatzdatum <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="datum"
          placeholder="Wann wird eingeplant?"
          mode="date"
    ...
    'mitarbeiter': (
      <div key="mitarbeiter" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel="Neu in Mitarbeiter"
        />
    ...
    'schichtvorlage': (
      <div key="schichtvorlage" className="space-y-1.5">
        <Label htmlFor="schichtvorlage">Schichtvorlage <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="schichtvorlage"
          placeholder="Schichtvorlage auswählen"
          items={schichtvorlagenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.schichtname ?? r.record_id),
          }))}
          value={extractRecordId(fields.schichtvorlage)}
          onChange={id => setFields(f => ({ ...f, schichtvorlage: id ? createRecordUrl(APP_IDS.SCHICHTVORLAGEN, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateSchichtvorlagen("schichtvorlage", q)}
          createLabel="Neu in Schichtvorlagen"
        />
        {showErrors && !fields.schichtvorlage && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'abwesend'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'abwesend' ? undefined : 'abwesend') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'abwesend'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'vertreten'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'vertreten' ? undefined : 'vertreten') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'vertreten'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Vertretung, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"datum": "Einsatzdatum", "mitarbeiter": "Mitarbeiter", "schichtvorlage": "Schichtvorlage", "status": "Status", "notizen": "Notizen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitarbeiter": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse", "abteilung": "Abteilung", "position": "Position / Rolle", "beschaeftigungsart": "Beschäftigungsart", "bemerkung": "Bemerkungen"}, "schichtvorlage": {"schichtname": "Schichtname", "schichttyp": "Schichttyp", "startzeit": "Startzeit", "endzeit": "Endzeit", "bereich": "Bereich / Abteilung", "mindestbesetzung": "Mindestbesetzung (Personen)", "beschreibung": "Beschreibung"}};
  ...
  return (
    ...
    {createSchichtvorlagenOpen && (
      <SchichtvorlagenDialog
        open={createSchichtvorlagenOpen}
        onClose={() => setCreateSchichtvorlagenOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createSchichtvorlagenEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Schichtvorlagen;
            setExtraSchichtvorlagen(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.SCHICHTVORLAGEN, result.id);
            setFields(prev => ({ ...prev, [createSchichtvorlagenField]: url } as any));
          }
          setCreateSchichtvorlagenOpen(false);
        }}
        defaultValues={createSchichtvorlagenInitial
          ? ({ schichtname: createSchichtvorlagenInitial } as any)
          : undefined}
      />
    )}
    </>
