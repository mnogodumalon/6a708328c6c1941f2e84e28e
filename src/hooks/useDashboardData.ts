...
export function useDashboardData() {
  ...
  const fetchAll = useCallback(async () => {
    setError(null);
    try {
      const [mitarbeiterData, schichtvorlagenData, schichtplanData] = await Promise.all([
  ...
  return { mitarbeiter, setMitarbeiter, schichtvorlagen, setSchichtvorlagen, schichtplan, setSchichtplan, loading, error, fetchAll, mitarbeiterMap, schichtvorlagenMap };
}