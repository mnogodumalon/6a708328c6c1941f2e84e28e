...
import type { Mitarbeiter, Schichtvorlagen, Schichtplan, CreateMitarbeiter, CreateSchichtvorlagen, CreateSchichtplan } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getSchichtvorlagenEntry(id: string): Promise<Schichtvorlagen | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schichtvorlagen;
    return enrichLookupFields([record], 'schichtvorlagen')[0];
  }
  static async createSchichtvorlagenEntry(fields: CreateSchichtvorlagen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHICHTVORLAGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtvorlagen') });
    return { ...data, record_id: data.id };
  }
  static async updateSchichtvorlagenEntry(id: string, fields: Partial<CreateSchichtvorlagen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHICHTVORLAGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtvorlagen') });
    return { ...data, record_id: data.id };
  }
  ...
  static async getSchichtplan(): Promise<Schichtplan[]> {
    ...
    return enrichLookupFields(records, 'schichtplan');
  }
  static async getSchichtplanEntry(id: string): Promise<Schichtplan | undefined> {
    ...
    return enrichLookupFields([record], 'schichtplan')[0];
  }
